...
uint16_t crc16_CITT (uint8_t *inData, uint16_t len) {
    ...
    while (len--)
        crc = (crc << 8) ^ crc16_table[(crc >> 8) ^ *inData++];
    return crc;
}
...
uint32_t crc32_CCITT (uint8_t *buf, uint32_t len) {
...
void crc32_CCITT_s (uint32_t *crc, uint8_t *buf) {
    *crc = (*crc >> 8) ^ (uint32_t)crc32_table[((uint32_t)*crc ^ (uint32_t)*buf++) & 0xFF];
}
...
//http://git.rockbox.org/?p=rockbox.git;a=blob;f=firmware/common/crc32.c;h=c8c70e415c613856b966eabe6a249b4568bf2a0a;hb=HEAD
#endif
